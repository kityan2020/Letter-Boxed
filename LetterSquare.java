 ...
 * Partner: Annie Liang (aaliang@bu.edu)
 * code added by: Kit Chung Yan
 */
...
public class LetterSquare {
    ...
    private boolean isValid(String letter, int wordNum, int charNum) {        
        if (charNum == 0){
            ...
            String r = this.words[wordNum] + letter;
            if (this.alreadyUsed(r)) {
                return false;
            }else{
                if (this.onSameSide(letter, lastLetter(this.words[wordNum])) == false){
                    if (dictionary.hasFullWord(r) || dictionary.hasString(r)) {
                        return true;
                    }
                }
            }
    ...
    private boolean solveRB(int wordNum, int charNum, int maxWords) {
        ...
        if (dictionary.hasFullWord(this.words[wordNum]) && this.words[wordNum].length() >= 3 && this.allLettersUsed()){
